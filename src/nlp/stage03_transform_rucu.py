...
def run_transform(
    ...
    date_match = re.search(r"\d{1,2} \w+ \d{4}", date_submitted_str)
    if date_match:
        parsed_date = datetime.strptime(date_match.group(), "%d %b %Y")
        LOG.info(f"Parsed submission date: {parsed_date.date()}")
    else:
        parsed_date = None
        LOG.warning("Could not parse submission date")

    # Calculate derived field: days since submission
    if parsed_date:
        days_since_submission: int = (datetime.today() - parsed_date).days
    else:
        days_since_submission: int = -1
    LOG.info(f"Calculated days since submission: {days_since_submission}")
