...
def run_validate(
    ...
    primary_subject_span = soup.find("span", class_="primary-subject")
    ...
    LOG.info("VALIDATE: DOI link found: %s", doi_link is not None)
    LOG.info("VALIDATE: Submission history found: %s", submission_history is not None)
    LOG.info(
        "VALIDATE: Primary subject span found: %s", primary_subject_span is not None
    )
    ...
    if not submission_history:
        missing.append("submission_history")
    if not primary_subject_span:
        missing.append("primary_subject")
